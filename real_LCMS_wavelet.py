...
class LCMS_WaveletDeconvolver:
    ...
    def _detect_blobs(self, response_map):
        """
        Zero‐out everything below a fixed wavelet‐response threshold,
        then run blob_log only on the high‐response pixels.
        """
        max_resp = np.max(response_map)

        # Throw away everything below about 2 ×10^6 (≈20% of a 1×10^7 peak)
        response_cutoff = 2e6
        mask = response_map < response_cutoff
        response_map_thresholded = response_map.copy()
        response_map_thresholded[mask] = 0

        # Now run blob_log “blindly” (threshold=0) on only the high‐response pixels:
        blobs = blob_log(
            response_map_thresholded,
            min_sigma=4,   # ignore anything narrower than ~4px
            max_sigma=8,   # ignore anything broader than ~8px
            threshold=0    # we already zeroed out below response_cutoff
        )
